...
function handleServerRequest(request, response) {
	...
	switch (urlParts.pathname) {
		...
		//BT: GET THE SLOWEST MINER HASH RATE
		case '/get_slowestminer':
			handleSlowestMiner(response);
			break;
		...
		case '/test_discord_notification':
			...
			handleTestDiscordNotification(urlParts, response);
			break;
		// Default response
		default:
 ...
 * Return top 10 miners
 **/
 function handleSlowestMiner(response) {
	async.waterfall([
		function (callback) {
			redisClient.keys(`${config.coin}:workers:*`, callback);
		},
		...
		data.sort(compareSlowestMiners);
		data = data.slice(0, 1);
		let reply = JSON.stringify(data);
...
function compareSlowestMiners(a, b) {
	let v1 = a.hashrate ? parseInt(a.hashrate) : 0;
	let v2 = b.hashrate ? parseInt(b.hashrate) : 0;
	if (v1 > v2) return 1;
	if (v1 < v2) return -1;
...
/**
 * Administration: test discord notification
 **/
function handleTestDiscordNotification(urlParts, response) {
	if (!config.discord) {
		response.end(JSON.stringify({
			status: 'Discord is not configured'
		}));
		return;
	}
	if (!config.telegram.enabled) {
		response.end(JSON.stringify({
			status: 'Discord is not enabled'
		}));
		return;
	}
	if (!config.telegram.token) {
		response.end(JSON.stringify({
			status: 'No Discord bot token specified in configuration'
		}));
		return;
	}
	if (!config.telegram.channel) {
		response.end(JSON.stringify({
			status: 'No Discord channel specified in configuration'
		}));
		return;
	}
	log('info', logSystem, 'Sending test discord channel notification');
	notifications.sendToDiscordChannel('test', {});
	response.end(JSON.stringify({
		status: 'done'
	}));
}
...
const test = 10;
//BT: EXPORT THE HASRATE
module.exports = {getHashRate, test, getStats};
