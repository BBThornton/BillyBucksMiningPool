...
function runInterval() {
	async.waterfall([
		...
		function (blocks, callback) {
			//BT: This gets the number of shares submitted by users for each block that is getting rewarded
		...
		function (blocks, callback) {
			...
			blocks.forEach(function (block) {
				...
				notifications.sendToAll('blockOrphaned', {
					...
					'SHARES': block.shares,
					'EFFORT': Math.round(block.shares / billy.Threshold * 100) + '%'
				})
			})
		...
		// //BT GET ALL the async valuyes i need
		// function(blocks, callback){
		// 	billy.setHistoricThreshold(10,100);
		// 	//BT: LOOP AND ADD COMMANDS TO LIST THE EXECUTE THEM ALL AND CALLBACK?
		// 	redisClient.zrangebyscore(config.coin + ':Threshold', 9, 11, 'WITHSCORES', function (error, results) {
		// 		if (error) {
		// 			log('error', logSystem, 'Error trying to get pending blocks from redis %j', [error]);
		// 			callback(true);
		// 			return;
		// 		}
		// 		if (results.length === 0) {
		// 			log('info', logSystem, 'TESTTESTTES');
		// 			callback(true);
		// 			return;
		// 		}
		// 		log('info', logSystem, JSON.stringify(results));

		// 		let thresholds = [];
		// 		thresholds.push(results);
		// 		for (let i = 0; i < results.length; i += 1) {
		// 			log('info', logSystem, 'HI %s', [JSON.stringify(results[i])]);
		// 			thresholds.push(results[i]);
		// 		}
		// 		callback(null, blocks, thresholds);
		...
		function (blocks,callback) {
			...
			//BT: GET THE SHARES SUBMITTED BY THE LOWEST HASRATE WORKER FROM THE PREVIOUS BLOCK				
			log('info', logSystem, JSON.stringify(api.getHashRate()));

			// let previousLowestSubmissions = redisClient.sendCommand(['hgetall',config.coin+':scores:previousThreshold'])[0] || 0;
			blocks.forEach(function (block) {
				if (block.orphaned) {
					return;
				}
				...
				if (Object.keys(donations).length) {
					for (let wallet in donations) {
						...
						feePercent += percent;
						payments[wallets] = Math.round(block.reward * percent);
				...
				if (block.workerScores) {
					...
					// log('info', logSystem, 'THRESHOLD: %d',[previousLowestSubmissions]);
					...
					if (block.rewardType === 'solo') {
						...
						/**
						 * BillyBucks:
						 * In order to balance the mining pool the following code implements the Balanced-reward function
						 * This function operates by equally distributing the rewards to all miners who met the threshold number of shares 
						 */

						let threshold = billy.Threshold;
						//Get the number of users greater than the threshold
						var filtered = Object.entries(block.workerScores).filter(function(itm){
							// console.log("Num Shares: %d",parseInt(itm[1]));
							// console.log("Threshold: %d", threshold);
							// console.log(parseInt(itm[1])>threshold);
							if (parseInt(itm[1])>threshold/2){
								return true;
							}; //Get all workers of greater then threshold number of shares
							return false
						})
												
						var num_eligable = Object.keys(filtered).length;
						
						console.log("NUM ELIGABLE:")
						console.log(num_eligable)
						
						if(num_eligable == 0){ ///Catch when the miner has mined a block before becoming eligable for a reward
							num_eligable = 1
						}
						

						Object.keys(block.workerScores).forEach(function (worker) {
							console.log(block.workerScores[worker])
							log('info', logSystem, 'WORKER: %s', [JSON.stringify(worker)]);
							let workerReward = 0;
							if  (block.workerScores[worker] > threshold){
								//reward = totalreward/num_miners
								workerReward = Math.round(reward/num_eligable);
								console.log(num_eligable);
								console.log(reward);
								console.log(workerReward);
								log('info', logSystem, 'NUM ELIGABLE %d REWARD',[num_eligable]);
								log('info', logSystem, 'PRE ADJUST %d REWARD',[reward]);

								log('info', logSystem, 'PROP Block %d REWARD',[workerReward]);
							}else if(block.login === worker){ //This worker was the individual who found the block
								workerReward = Math.round(reward/num_eligable);
							}

							
							//Pay the workers the worker reward
							payments[worker] = block.login === worker ? (payments[worker] || 0) + (workerReward + finderReward) : (payments[worker] || 0) + workerReward;
							log('info', logSystem, 'PROP Block %d payment to %s for %d%% of total block score: %d', [block.height, worker, 1/num_eligable*100, payments[worker]]);
						});
					}
				}

				//BT: Update the lowest hashes to the current blocks lowest hash rate
