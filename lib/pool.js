...
let billy = require('./billybucksfunctions')
let sharecount = 0;
...
setInterval(function () {
	...
	for (let minerId in connectedMiners) {
		...
		console.log(miner.difficulty)
	}
}, config.poolServer.varDiff.retargetTime * 1000);

setInterval(function(){
	// log('info', logSystem, 'CURRENT MINERS:');
	// log('info', logSystem, JSON.stringify(connectedMiners));
}, 3000)
...
function notifyConnectedMiners (indexOfChildPool) {
	let now = Date.now() / 1000 | 0;
	for (let minerId in connectedMiners) {
		//BT: RESET THE MINER COUNT OF SHARES
		let miner = connectedMiners[minerId];
...
Miner.prototype = {
	retarget: function (now) {
		...
		//Get the new share diff from the billy bucks function library
		const NewShareDiff = billy.getShareDiff();
		let sinceLast = now - this.lastShareTime;
		let decreaser = sinceLast > VarDiff.tMax;
		let newDiff = NewShareDiff;
		
		//Set mining difficulty to the new share diff
	...
	pause: function(){
		/**
		 * BillyBucks:
		 * This function sends a pause job (Job ID 10) to miners if they are eligable for throttling
		 * due to reaching the pre-requisite number of shares
		 */
		let blockTemplate = currentBlockTemplate[this.activeChildPool];
		let newJob = {
			id: "10",
...
function handleMinerMethod (method, params, ip, portData, sendReply, pushMessage) {
	...
	switch (method) {
		case 'login':
			...
			log('info', logSystem, 'ADDED MINER');
			log('info', logSystem, JSON.stringify(connectedMiners));
...
function newConnectedWorker (miner) {
	log('info', logSystem, 'newConnectedWorker');
...
function removeConnectedWorker (miner, reason) {
	redisClient.hincrby(`${config.coin}:ports:${miner.port}`, 'users', '-1');
	log('info', logSystem, 'removeConnectedWorker');
...
function recordShareData (miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate, pool, sendReply) {
	//BT: WHERE THE SHARE DATA IS UPDATED
	...
	if (slushMiningEnabled) {
		...
		//BillyBucks this used to increment the count by the job difficulty for the price per share reward function
		//Now ill just increment by one every share s
	...
	let redisCommands = [
		...
		//BillyBucks increment by 1 instead of the job difficulty
		['hincrby', `${coin}:shares_actual:${rewardType}:roundCurrent`, login, 1],
	...
	if (blockCandidate) {
		//BT: HERE A NEW BLOCK HAS BEEN SUBMITTED
	...
	redisClient.multi(redisCommands)
		.exec(function (err, replies) {
			...
			if (blockCandidate) {
				...
				/**
				 * BillyBucks:
				 * Sets the lowest Hashrate Miner of found block
				 */
				billy.setLowestHashrateMiner(coin);

				
			}
			/**
			 * BillyBucks:
			 * This function is responsible for pausing miners that have gone betond the threshold number of shares
			 * Currently this may result in the lowest hashrate miner mining (potentially) for significantly longer due to probablistic mining
			 */

			//Get the current share count of the current miner
			redisClient.hget(`${coin}:scores:${rewardType}:roundCurrent`, login, function (err, result) {
				if (err) {
					log('error', logSystem, 'Failed To get num shares for miners');
				}else{
					console.log("Current Share Count %d", result);
					var hashes = result; //The share count of the current miner
					redisClient.hget(`${coin}:hashrateMiners`, "lowest", function (err, result) { //Get the lowest hashrate miner from the redux database
						if (err) {
							console.log('error');
							
						}else{
							console.log("Lowest Hashrate Miner: %s", result);
							if(result == null){ //If no miner set in the database then update the store
								return setLowestHashrateMiner(coin);
							}
							var LowestMiner = result;

							console.log("Current Miner: %s",login);
							//If the current miner is not the lowest hashrate miner and is over the threshold sent the "pause" job
							if(hashes>billy.Threshold && login!==LowestMiner){
								log('info', logSystem, 'Pausing miner %s at Threshold',[login]);
	...
	if (hashDiff.ge(blockTemplate.difficulty)) {
		...
		apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function (error, result) {
			if (error) {
				...
				recordShareData(miner, job, hashDiff.toString(), true, blockFastHash, shareType, blockTemplate, null, sendReply);
			}
		});
		...
		recordShareData(miner, job, hashDiff.toString(), false, null, shareType, null, null, sendReply);
	}
			...
			if (hashDiff.ge(childBlockTemplate.difficulty)) {
				...
				if (mergedBuffer === null) {
					...
					let onChildSuccess = (result) => {
						...
						recordShareData(miner, job, hashDiff.toString(), true, blockFastHash, shareType, childBlockTemplate, pool, sendReply);
					}
				...
				recordShareData(miner, job, hashDiff.toString(), false, null, shareType, null, pool, sendReply);
			}
		}
		return true;
	}
	...
	async.each(config.poolServer.ports, function (portData, cback) {
		let handleMessage = function (socket, jsonData, pushMessage) {
			...
			let sendReply = function (error, result) {
				...
				log('warn', logSystem, 'Outgoing Message: %s',[sendData]);
				socket.write(sendData);
			};

			log('warn', logSystem, JSON.stringify(jsonData));
